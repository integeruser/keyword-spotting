...
def pcr():
    if len(sys.argv) != 3:
        sys.exit('Usage: {0} pcr corpus_filepath'.format(sys.argv[0]))

    corpus_filepath = sys.argv[2]
    print('Starting pcr...')
    print('   {0: <16} = {1}'.format('corpus_filepath', corpus_filepath))

    ########################################################################

    print('Corpus info:')
    corpus = utils.load_corpus(corpus_filepath)
    ...
    print('   {0: <20} = {1}'.format('keypoints per page',
                                     [len(page_keypoints) for page_keypoints in corpus['keypoints']]))
    print('   {0: <20} = {1}'.format('descriptors per page',
                                     [len(page_descriptors) for page_descriptors in corpus['descriptors']]))


def dcr():
    if len(sys.argv) != 4:
        sys.exit('Usage: {0} dcr corpus_filepath page_file_path'.format(sys.argv[0]))
    ...
    cv2.imshow('page',
               cv2.drawKeypoints(page_image, page_keypoints, None, flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS))
    cv2.waitKey()


def pcd():
    if len(sys.argv) != 3:
        sys.exit('Usage: {0} pcd codebook_filepath'.format(sys.argv[0]))

    codebook_filepath = sys.argv[2]
    print('Starting pcd...')
    print('   {0: <18} = {1}'.format('codebook_filepath', codebook_filepath))

    ########################################################################

    print('Codebook info:')
    codebook = load_codebook(codebook_filepath)

    print('   {0: <23} = {1}'.format('corpus_filepath', codebook['corpus_filepath']))
    ...
    print('   {0: <23} = {1}'.format(
        'keypoints count',
        sum([len(v) for codeword in codebook['codewords'] for v in codeword['keypoints'].viewvalues()])))
    print('   {0: <23} = {1}'.format(
        'keypoints per codewords',
        [len(v) for codeword in codebook['codewords'] for v in codeword['keypoints'].viewvalues()]))


def pmt():
    if len(sys.argv) != 3:
        sys.exit('Usage: {0} pmt matches_filepath'.format(sys.argv[0]))

    matches_filepath = sys.argv[2]
    print('Starting pmt...')
    print('   {0: <17} = {1}'.format('matches_filepath', matches_filepath))

    ########################################################################

    print('Matches info:')
    matches = load_matches(matches_filepath)
...
def codeword_vs_query():
    if len(sys.argv) != 4:
        sys.exit('Usage: {0} codeword_vs_query pages_directory_path query_file_path'.format(sys.argv[0]))
    ...
    while True:
        ...
        cv2.imshow(
            'codeword-keypoints{0}-{1}'.format(page_index, query_keypoint_index),
            cv2.drawKeypoints(
                page_image,
                utils.namedtuple_keypoints_to_cv2(codeword_keypoints),
                None,
                flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS))
        cv2.moveWindow('codeword-keypoints{0}-{1}'.format(page_index, query_keypoint_index), 200, 200)

        cv2.imshow(
            'query-keypoints{0}-{1}'.format(page_index, query_keypoint_index),
            cv2.drawKeypoints(
                query_image,
                utils.namedtuple_keypoints_to_cv2([query_keypoint]),
                None,
                flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS))
